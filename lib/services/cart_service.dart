...
class CartService {
  ...
  Future<bool> addToCart({
    ...
    try {
      ...
      final cartRef = _firestore.collection('carts').doc(currentUserId);
      final itemRef = cartRef.collection('items').doc(productId);

      // DEBUG: Using simplified write instead of transaction to isolate the issue.
      ...
      if (!cartDoc.exists) {
        ...
        print('DEBUG: Cart document created because it did not exist.');
      }

      // Now, set the item data, merging with existing data if any.
      await itemRef.set({
        'productId': productId,
        'name': productName,
        'imageUrl': imageUrl,
      ...
      print('DEBUG: Successfully wrote item to cart with simplified method.');

      return true;
    } catch (e, s) {
      print('Error adding to cart in service: $e');
      print('Stack trace in service: $s');
